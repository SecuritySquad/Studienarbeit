...
\lstset{
    style=eclipse,
    caption={\textit{webifier Tester} - Hilfe},
...
\lstset{
    style=eclipse,
    caption={\textit{webifier Tester} - Standardausgabe},
...
\lstset{
    style=eclipsejavascript,
    caption=[\textit{webifier Tester} - Ausschnitt Konfigurationsdatei]{\textit{webifier Tester} -
    Ausschnitt Konfigurationsdatei\protect\footnotemark}, label={lst:tester-config}
}
\begin{lstlisting}
{
  "resolver": {
...
\lstset{
    style=eclipsejava,
    caption=[\textit{webifier Tester} - Ausschnitt Ergebnisberechnung]{\textit{webifier Tester} -
    Ausschnitt Ergebnisberechnung\protect\footnotemark}, label={lst:tester-testresult-calculation}
}
\begin{lstlisting}
...
\lstset{
    style=eclipsejavascript,
    caption={\textit{webifier Plattform} - Konfigurationsdatei},
...
Die Detailansicht des \textit{Google Safe Browsing} Ergebnisses ist ähnlich dem der Prüfung aller
...
Der Screenshot der Seite wird beim Ausklappen des Panels einfach angezeigt, so wie es in Abbildung \ref{fig:platform-result-screenshot} dargestellt. Dies gibt dem Nutzer die Möglichkeit sich die Webseite anzusehen, ohne sie selbst zu besuchen.
...
\lstset{
    style=eclipsejavascript,
    caption={\textit{webifier Data} - Ausschnitt Inhalt push-Request},
    label={lst:data-push}
}
\begin{lstlisting}
{
...
\lstset{
    style=eclipsejavascript,
    caption={\textit{webifier Data} - Inhalt check-Request},
...
\lstset{
    style=eclipsejava,
    caption={\textit{webifier Data} - Verarbeitung check Methode},
...
\lstset{
    style=eclipsejavascript,
    caption={\textit{webifier Data} - Inhalt check-Response},
...
\lstset{
    style=eclipsejavascript,
    caption={\textit{webifier Data} - Beispieldokument webifierTestResultData},
...
\textit{webifier Statistics} wird in R implementiert. Hierzu werden
...
\begin{enumerate}
  ...
  \item Entsprechende API für Generierung der Grafik ansteuern
\end{enumerate}

\begin{scriptsize}
\lstset{
    style=eclipsejavascript,
    caption={\textit{webifier Statistics} - Beispiel R-Grafik},
    label={lst:rgrafik}
}
\begin{lstlisting}
...
muss nun nur noch der Text, die Farbe und ein passendes Icon ausgewählt werden. Die Generierung und
...
Zeile 31).
Diese Datei enthält eine Liste von Browserobjekten.
...
Danach werden für alle Konfigurationen die entsprechenden Header ermittelt.
...
Der Ablauf des Tests sieht wie folgt aus: Zunächst wird Bro intialisiert und es werden Filter
...
Liste von riskanten Ports gespeichert. Die Anzahl an Ports in dieser Liste bestimmt nun das Ergebnis
des Tests. Wurden keine verdächtigen Portanfragen gefunden, wird das Ergebnis \textit{unbedenklich}
...
gespeichert. Die Anzahl der Elemente in dieser Liste bestimmt das Ergebnis des Tests. Hierbei sind
...
Ihm wird die zu testende \acs{URL} der Webseite mitgeliefert.
...
Nutzt die Webseite kein Zertifikat, so ist das Ergebnis \textit{SUSPICIOUS}. Hat die Webseite ein
...
Der vierte Schritt ist das Abrufen der Links der möglichen Originalwebseiten. Wie im ersten Schritt
