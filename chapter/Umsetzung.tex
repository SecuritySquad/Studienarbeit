...
Daraufhin schickt der Browser eine \ac{HTTP}-GET Anforderung an \textit{webifier Plattform}, den
Webserver der Anwendung.
Dort wird die Anfrage angenommen und durch das Spring Web Framework verarbeitet.
Dabei wird ein neuer \textit{Tester}-Prozess mit der zu prüfenden \ac{URL} über \ac{CLI} gestartet.
...
Dazu wird wie bei der \textit{webifier Plattform} pro \ac{URL} ein Prüfdurchlauf beim Tester über \ac{CLI} angefordert.
...
Das Ergebnis der Prüfung aller verlinkten Seiten enthält eine einfache Liste mit allen gefundenen
